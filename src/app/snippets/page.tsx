...
function SnippetsPage() {
  ...
  // State variables to store the user's search query and selected language
  const [searchQuery, setSearchQuery] = useState("");
  const [selectedLanguage, setSelectedLanguage] = useState<string | null>(null);

  // State variable to store the current view: grid or list
  const [view, setView] = useState<"grid" | "list">("grid");

  // If the snippets are still loading, show a skeleton
  ...
  // Get a list of unique languages from the snippets
  const languages = [...new Set(snippets.map((s) => s.language))];

  // Only show the 5 most popular languages
  const popularLanguages = languages.slice(0, 5);

  // Filter the snippets based on the user's search query and selected language
